import java.util.List;
import java.util.ArrayList;

public class DeckArray {

	/**
	 * The Deck class represents a shuffled deck of cards.
	 * It provides several operations including
	 *      initialize, shuffle, deal, and check if empty.
	 */

		/**
		 * cards contains all the cards in the deck.
		 */
		//private List<Card> cards; created with array only 
		private Card[] cardArr;

		/**
		 * size is the number of not-yet-dealt cards.
		 * Cards are dealt from the top (highest index) down.
		 * The next card to be dealt is at size - 1.
		 */
		private int size;


		/**
		 * Creates a new <code>Deck</code> instance.<BR>
		 * It pairs each element of ranks with each element of suits,
		 * and produces one of the corresponding card.
		 * @param ranks is an array containing all of the card ranks.
		 * @param suits is an array containing all of the card suits.
		 * @param values is an array containing all of the card point values.
		 */
		public DeckArray(String[] ranks, String[] suits, int[] values) {
			/* *** TO BE IMPLEMENTED IN ACTIVITY 2 *** completed 1a*/
			size = ranks.length*suits.length;
			cardArr = new Card[size];
			for (int i = 0; i<ranks.length; i++)
			{
				for(int j = 0; j<suits.length; j++)
				{
					
					Card myCard = new Card(ranks[i], suits[j], values[i]);
					cardArr[suits.length*i+j] = myCard;
				}
			}
			shuffle();
		}


		/**
		 * Determines if this deck is empty (no undealt cards).
		 * @return true if this deck is empty, false otherwise.
		 */
		public boolean isEmpty() {
			/* *** TO BE IMPLEMENTED IN ACTIVITY 2 *** completed 1b */
			return (size == 0);
		}

		/**
		 * Accesses the number of undealt cards in this deck.
		 * @return the number of undealt cards in this deck.
		 */
		public int size() {
			/* *** TO BE IMPLEMENTED IN ACTIVITY 2 *** completed 1c */
			return size;
		}

		/**
		 * Randomly permute the given collection of cards
		 * and reset the size to represent the entire deck.
		 */
		public void shuffle() {
			/* *** TO BE IMPLEMENTED IN ACTIVITY 4 *** */
			for (int k = cardArr.length-1; k>0; k--)
			{
				int r = (int) (Math.random()*(k+1));
				Card temp = cardArr[k];
				cardArr[k] = cardArr[r];
				cardArr[r] = temp;
				size = cardArr.length;
			}
		}

		/**
		 * Deals a card from this deck.
		 * @return the card just dealt, or null if all the cards have been
		 *         previously dealt.
		 */
		public Card deal() {
			/* *** TO BE IMPLEMENTED IN ACTIVITY 2 *** completed 1d */
			if (isEmpty())
			{
				return null;
			}
			size--;
			Card c = cardArr[size];
			return c;
		}

		/**
		 * Generates and returns a string representation of this deck.
		 * @return a string representation of this deck.
		 */
		@Override
		public String toString() {
			String rtn = "size = " + size + "\nUndealt cards: \n";

			for (int k = size - 1; k >= 0; k--) {
				rtn = rtn + cardArr[k];
				if (k != 0) {
					rtn = rtn + ", ";
				}
				if ((size - k) % 2 == 0) {
					// Insert carriage returns so entire deck is visible on console.
					rtn = rtn + "\n";
				}
			}

			rtn = rtn + "\nDealt cards: \n";
			for (int k = cardArr.length - 1; k >= size; k--) {
				rtn = rtn + cardArr[k];
				if (k != size) {
					rtn = rtn + ", ";
				}
				if ((k - cardArr.length) % 2 == 0) {
					// Insert carriage returns so entire deck is visible on console.
					rtn = rtn + "\n";
				}
			}

			rtn = rtn + "\n";
			return rtn;
		}
		
		public String printUndealt()
		{
			String rtn = "";
			for (int k = size - 1; k >= 0; k--) {
				rtn = rtn + cardArr[k];
				if (k != 0) {
					rtn = rtn + ", ";
				}
				if ((size - k) % 2 == 0) {
					// Insert carriage returns so entire deck is visible on console.
					rtn = rtn + "\n";
				}
			}
			if (rtn.contentEquals(""))
			{
				rtn = "No undealt cards remain";
			}
			return rtn;
		}
	}
